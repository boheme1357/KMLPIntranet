	...
	<div id="wapper">
		...
		<sf:form action="register">
			...
			<table border="1" width="100%">
				...
				<c:forEach var="row" items="${map.list}">
					<tr>
						...
						<td>${row.n_id}</td>
						<td><a
							href="${path}/notice/nContent.do?n_num=${row.n_num}&curPage=${map.boardPager.curPage}&searchOption=${map.searchOption}&keyword=${map.keyword}">${row.n_title}</a></td>
						<td>${row.n_file}</td>
						<td>${row.n_wdate}</td>
						<td>${row.n_viewcnt}</td>
					</tr>
			...
			<sec:authorize access="hasRole('ADMIN')">
				<div class="w3-center">
					<input type="button" id="button" class="button" value="공지사항 등록"
						onclick="location.href='${path}/notice/nRegist.do'">
				</div>
			</sec:authorize>
		</sf:form>
	</div>
